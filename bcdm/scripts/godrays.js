...
function drawPlanes(context,r,id){
	...
	if(r.colored)
		grd.addColorStop(1,GetColorString(r.color));
	else
		grd.addColorStop(1,'rgba(0,0,0,0.77)');
...
function drawRays(context,r){
	...
	grd.addColorStop(0,'rgba(0,0,0,0)');
	var red = Math.random()*120+125;
	grd.addColorStop(1,'rgba('+red+',0,0,0.3)');
...
function GodRays(nb,in_x,in_y,in_mouse)
{
	this.type = "godrays";
	...
	for(var i=0;i<this.nb;i++)
	{
		...
		this.rays[i] = new ray(nbr,xx,yy,this.hw*2,Math.random()*this.hw/4,3*this.hw/4,true,i);
