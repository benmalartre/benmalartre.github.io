...
function Agent(ctxt,id,x,y,sx,sy){
	...
	this.SetColor = function(r,g,b){
		this.colorr = Math.floor(r);
		this.colorg = Math.floor(g);
		this.colorb = Math.floor(b);
	}
	...
	this.Chase = function(weight)
	{
		this.Wander(weight);
		this.tmp.Sub(this.pos,center);
		//this.tmp.NormalizeInPlace();
		this.BlendVelocity(this.tmp,weight);
	...
	this.Draw = function()
	{
		...
		//this.ctxt.fillStyle = get_random_color();//'rgb('+Math.random()*255+','+Math.random()*255+','+Math.random()*255+')';
		...
		var str = 'rgba('+this.colorr+','+this.colorg+','+this.colorb+',1)';
		this.ctxt.fillStyle = str;
...
function Flock(nbAgents,in_width,in_height)
{
	...
	this.agents = new Array(nbAgents+1);
	var r1,r2;
	center.Set(this.width*0.5,this.height*0.5);
	this.hero = new Agent(this.ctxt,-1,10,10,10,10);
	this.hero.chasing = true;
	this.hero.SetColor(255,0,0);
	this.lastx = 0.0;
	this.lasty = 0.0;
	for(var i=0;i<this.nb;i++)
	{
		...
		this.agents[i].SetColor(Math.random()*50,Math.random()*100+150,Math.random()*50);
	}
	this.agents[this.nb] = this.hero;
	this.nb = this.nb+1;
	this.grid = new Grid(this.canvas,12,10);
	
	this.Light = function(_mx,_my){
		
		var grd = this.ctxt.createRadialGradient(this.lastx,this.lasty,this.width/100,this.lastx,this.lasty,this.width/2);
		grd.addColorStop(0,'rgba(0,0,0,0)');
		grd.addColorStop(0.5,'rgba(0,0,0,0.66)');
		grd.addColorStop(1,'rgba(0,0,0,0.95)');
		this.ctxt.fillStyle = grd;
		this.ctxt.fillRect(0,0,this.width,this.height);
		this.lastx = _mx*0.1+this.lastx*0.9;
		this.lasty = _my*0.1+this.lasty*0.9;
	}
	...
	this.Update = function(mx,my)
	{
		...
		this.ctxt.clearRect(0,0,this.width,this.height);
		var _mx = this.hero.pos.x;//mx*this.width;
		var _my = this.hero.pos.y;//my*this.height;
		
		//this.hero.Wander(10);
		//this.hero.Update(0,0);
		//this.hero.Draw();
		for(var i=0;i<this.nb;i++)
		{
